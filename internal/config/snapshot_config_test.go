...
func TestSnapshotConfig_Env(t *testing.T) {
	...
	t.Setenv("SHINYHUB_RUNTIME_DOCKER_SNAPSHOT_ENABLED", "true")
	t.Setenv("SHINYHUB_RUNTIME_DOCKER_SNAPSHOT_MAX_SUSPENDED", "8")
	t.Setenv("SHINYHUB_RUNTIME_DOCKER_SNAPSHOT_RECLAIM_MIN_FRACTION", "0.75")

	cfg, err := config.Load("")
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	s := cfg.Runtime.Docker.Snapshot
...
func TestSnapshotConfig_RejectsBadFraction(t *testing.T) {
	t.Setenv("SHINYHUB_AUTH_SECRET", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
	t.Setenv("SHINYHUB_RUNTIME_DOCKER_SNAPSHOT_RECLAIM_MIN_FRACTION", "1.5")
