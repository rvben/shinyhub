// Multi-replica application log viewer. The backend still stores one append-only
// file per replica slot; this module makes that contract honest by identifying
// every line, retaining ended sources, and treating pause separately from scroll.
...
export function isLiveLogSource(source) {
  return LIVE_STATUSES.has(String(source && source.status || '').toLowerCase());
}
...
export function normalizeLogSources(raw = []) {
  ...
  for (const item of Array.isArray(raw) ? raw : []) {
    ...
    const previous = byReplica.get(replica) || {};
    byReplica.set(replica, {
      ...previous,
      ...item,
      replica,
  ...
  return [...byReplica.values()].sort((a, b) => a.replica - b.replica);
}
...
function sourceStatusEvent(source, previous) {
  if (!previous) return `Replica #${source.replica} discovered (${titleCase(source.status)})`;
...
export function createLogsViewer({
  ...
  try {
    ...
    if (requested === 'all' || /^\d+$/.test(requested || '')) selected = requested;
  } catch { /* non-browser test environment */ }

  function scopedSources() {
    if (selected === 'all') return sources;
    return sources.filter((source) => String(source.replica) === selected);
  }
  ...
  function renderSourceOptions() {
    ...
    const liveCount = sources.filter(isLiveLogSource).length;
    all.value = 'all';
    all.textContent = `All replicas (${liveCount} live, ${sources.length} retained)`;
    const options = [all];
    for (const source of sources) {
      const option = doc.createElement('option');
      option.value = String(source.replica);
      option.textContent = formatLogSourceLabel(source);
      options.push(option);
    }
    sourceSelect.replaceChildren(...options);
    selected = prior === 'all' || sources.some((s) => String(s.replica) === prior) ? prior : 'all';
  ...
  function updateConnectionStatus() {
    ...
    const connectedCount = followable.filter((source) => connected.has(source.replica)).length;
  ...
  async function loadStaticSource(source, generation) {
    ...
    try {
      const resp = await api(`/api/apps/${encodeURIComponent(app.slug)}/logs?replica=${source.replica}&tail=200&follow=false`);
  ...
  function openLiveSource(source, generation) {
    ...
    const url = `/api/apps/${encodeURIComponent(app.slug)}/logs?replica=${source.replica}&tail=200`;
    const stream = new EventSourceClass(url, { withCredentials: true });
    streams.set(source.replica, stream);
    stream.onopen = () => {
      if (destroyed || generation !== scopeGeneration) return;
      connected.add(source.replica);
    ...
    stream.onerror = () => {
      if (destroyed || generation !== scopeGeneration) return;
      connected.delete(source.replica);
      updateConnectionStatus();
    };
  }

  function loadSource(source, generation = scopeGeneration) {
    if (loadedSources.has(source.replica)) return;
    loadedSources.add(source.replica);
  ...
  function reconcileSources(nextSources, markChanges) {
    const previous = new Map(sources.map((source) => [source.replica, source]));
    sources = normalizeLogSources(nextSources);
    renderSourceOptions();
    if (markChanges) {
      for (const source of sources) {
        const old = previous.get(source.replica);
        ...
        if (selected !== 'all' && selected !== String(source.replica)) continue;
        if (old && isLiveLogSource(old) && !isLiveLogSource(source)) {
          const stream = streams.get(source.replica);
          if (stream) stream.close();
          streams.delete(source.replica);
          connected.delete(source.replica);
        } else if ((!old || !isLiveLogSource(old)) && isLiveLogSource(source)) {
          loadedSources.delete(source.replica);
  ...
  sourceSelect.addEventListener('change', () => {
    ...
    updateURL();
    resetScope();
    announce(selected === 'all' ? 'Showing all replica logs' : `Showing replica #${selected}`);
  });
  ...
  downloadButton.addEventListener('click', () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${app.slug}-logs-${selected === 'all' ? 'all' : `replica-${selected}`}.log`;
      a.click();
