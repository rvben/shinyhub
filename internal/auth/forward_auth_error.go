...
func writeForwardAuthProxyError(w http.ResponseWriter, r *http.Request, secretHeader string, missing bool) {
	...
	w.Header().Set("Content-Security-Policy", "default-src 'none'; style-src 'sha256-"+base64.StdEncoding.EncodeToString(styleHash[:])+"'; form-action 'self'; base-uri 'none'; frame-ancestors 'self'")
...
</html>`, forwardAuthProxyErrorCSS, html.EscapeString(lead), header, header)
}
