...
var schemaAnnotations = map[string]cmdAnnotation{
	...
	"schedule runs": {Mutating: ro, OutputFields: []fieldSpec{
		...
		{Name: "trigger", Type: "string", Desc: "schedule (cron boundary) | missed (startup catch-up) | register (run_on_register first fire) | manual"},
	...
	"fleet apply": {Mutating: mut, Cardinality: "bounded", OutputFields: []fieldSpec{
		...
		{Name: "schedule_verification", Type: "array", Desc: "enabled schedules rejected as stale by --verify-schedules"},
	...
	}, Notes: "Per-app results carry failure_kind (set when status is failed) and attempt_details (failed deploy attempts only) so deploy and post-deploy convergence failures are machine-classifiable. --wait-for-warm checks the run_on_register success level after every non-delete action, including unchanged apps, and fails closed when success cannot be established; --warm-timeout (default 15m) is one total deadline per app. If an unsatisfied warm gate has an active run, apply joins that run instead of dispatching duplicate work. --restart-after-warm implies --wait-for-warm and cycles serving replicas only after the final success check passes. --verify-schedules is read-only and requires every enabled schedule to be non-stale according to the server. --verify-health extends serving-health checks to unchanged apps; a parked app (hibernated, suspended) wakes on its first request and passes in one poll, and intentionally stopped apps remain excluded. Current servers sequence and heartbeat fleet runs so late older writes are rejected and interrupted runs are distinguishable from success. --concurrency (default 3, 1 = serial) bounds how many apps deploy in parallel; lower it on CPU- or memory-constrained hosts since concurrent uv sync / renv restore builds compete for resources. Every deployed app is health-waited for up to --health-timeout seconds whether or not --wait-for-warm is set; healthy means status running, or idle for an elastic (grouped/per_session) pool, which boots workers on demand and reports idle with none running. min_warm_replicas keeps multiplex replicas warm and is inert under elastic isolation (the server reports it as a manifest warning; use [app.worker] warm_spares to pre-boot elastic workers)."},
