...
var schemaAnnotations = map[string]cmdAnnotation{
	...
	"drafts delete":  {Mutating: boolp(true), Notes: "Removes the preview and retained source archive."},
	...
	"deploy": {
		...
		OutputFields: []fieldSpec{
			...
			{Name: "warm_restarted", Type: "boolean", Desc: "True when --restart-after-warm cycled serving replicas after bundle data convergence"},
			...
			{Name: "warning", Type: "string", Desc: "Present when deployment committed but schedule convergence requires asynchronous repair"},
			{Name: "warnings", Type: "array", Desc: "Server advisories about shinyhub.toml settings that were accepted but cannot take effect as written (for example min_warm_replicas under elastic worker isolation); omitted when there are none"},
		},
		Notes: "--open implies --start and --wait, verifies public apps through their user-facing route, and launches the default browser; browser-launch failure is non-fatal and leaves opened=false with a copyable URL. --watch requires an explicit --host or SHINYHUB_HOST, implies --start and --wait, debounces and coalesces local changes, skips unchanged bundle digests, keeps running after recoverable deployment failures, opens only after its first successful deployment, and attributes every attempt to one development session. The target slug must exist by default; --create explicitly creates a persistent app, and --ephemeral creates a private app with --ttl (8h default, 15m-7d) and a generated slug when omitted. --create and --ephemeral are mutually exclusive. --git is not supported, --output json is rejected, and manifests with post-deploy hooks require --allow-repeated-hooks because each deployable change reruns them. --restart-after-warm implies --wait-for-warm and cycles serving replicas only when convergence repaired data after those replicas were already running; deliberately stopped apps remain stopped outside watch mode. Deploy-triggered producers execute against the exact candidate bundle before its replicas boot. --output json emits one final result document. --output ndjson emits ordered phase records followed by exactly one result or error event per deployment attempt; watch mode also emits watch phase records and can emit several attempt results or errors before its final stopped event. Event types are phase, result, and error, and phase statuses are started, progress, completed, warning, and failed. Older servers fall back to a synthetic terminal event. shinyhub.toml [app] startup_timeout_seconds (1-3600, default 120) sets the readiness deadline the deploy health check allows before declaring the app crashed; readiness_path (absolute path, default /) selects the GET endpoint and readiness_status (100-599, omitted accepts 2xx/3xx) can require an exact response. These travel with the bundle and also apply on local run, wake, scale, and rollback. build_timeout_seconds (30-7200, default 900) bounds the host-side environment build (uv sync / renv restore) that runs before readiness; a build that exceeds it fails build_failed, distinct from startup_timeout_seconds, and is inert under the docker runtime. shinyhub.toml [app] also accepts memory_limit_mb (0 or 16-1048576) and cpu_quota_percent (0 or 1-6400; 100 = 1 core) - per-replica cgroup v2 ceilings reconciled into the app on deploy (declared-only, like replicas); 0 = explicit unlimited, omitted = unchanged. Clear back to inherit-global with `shinyhub apps set --memory-limit-mb -1` / `--cpu-quota-percent -1`.",
	},
	...
	"apps show": {Mutating: ro, OutputFields: []fieldSpec{
		...
		{Name: "replicas_status", Type: "array", Desc: "Replica state including data_generation and exact data_producer_{deployment_id,app_version,content_digest,fingerprint} when loaded by a schedule activation"},
		...
		{Name: "schedule_convergence_satisfied", Type: "boolean", Desc: "True when every enabled deploy-trigger policy is satisfied by the authoritative producer state"},
		{Name: "compatibility_quarantined", Type: "boolean", Desc: "True when consumer starts are blocked because a producer may have left shared data incompatible"},
		{Name: "producer_repair_required", Type: "boolean", Desc: "True when any schedule has an uncertain physical data write that requires successful producer repair"},
		{Name: "deploy_trigger_schedules", Type: "array", Desc: "Per-schedule desired obligation and authoritative last-writer provenance"},
	...
	"apps logs": {Mutating: ro, Streaming: true,
		Notes: "--run reads one immutable execution, --runs lists retained execution history, and --system wraps one-shot application output with recorded supervisor start/exit markers and terminal exit diagnostics."},
	"apps metrics": {Mutating: ro, OutputFields: []fieldSpec{
		...
		{Name: "pss_bytes", Type: "integer", Desc: "Proportional set size for the first running replica; null outside native Linux"},
		{Name: "uss_bytes", Type: "integer", Desc: "Private memory for the first running replica; null outside native Linux"},
		{Name: "swap_pss_bytes", Type: "integer", Desc: "Proportionally attributed swap for the first running replica; null outside native Linux"},
		{Name: "replicas", Type: "array", Desc: "Per-replica index, status, pid, cpu_percent, rss_bytes, pss_bytes, uss_bytes, swap_pss_bytes, sessions"},
	}},
	...
	"apps rollback": {Mutating: mut, OutputFields: []fieldSpec{
		...
		{Name: "schedule_convergence", Type: "array", Desc: "Exact deploy-trigger obligations for the rollback target, including prestart run provenance"},
	...
	"top": {Mutating: ro,
		...
		OutputFields: []fieldSpec{
			...
			{Name: "pss_bytes", Type: "integer", Desc: "PSS summed across running replicas; null when unavailable"},
			{Name: "pss_bytes_partial", Type: "boolean", Desc: "True when PSS is a lower bound"},
			{Name: "uss_bytes", Type: "integer", Desc: "Private memory summed across running replicas; null when unavailable"},
			{Name: "uss_bytes_partial", Type: "boolean", Desc: "True when private memory is a lower bound"},
			{Name: "swap_pss_bytes", Type: "integer", Desc: "Swap PSS summed across running replicas; null when unavailable"},
			{Name: "swap_pss_bytes_partial", Type: "boolean", Desc: "True when swap PSS is a lower bound"},
		...
		EnvelopeFields: []fieldSpec{
			...
			{Name: "totals", Type: "object", Desc: "Fleet sums for CPU, RSS, PSS, private memory, swap PSS, and sessions, with partial flags"},
		},
	...
	"schedule ls": {Mutating: ro, OutputFields: []fieldSpec{
		...
		{Name: "deploy_trigger", Type: "string", Desc: "never | first_deploy | bundle_change"},
		{Name: "effective_timezone", Type: "string"},
		{Name: "timezone_inherited", Type: "boolean"},
		{Name: "last_run_id", Type: "integer", Desc: "id of the most recent run; absent if never run; resolved atomically with last_run_status"},
		{Name: "last_run_at", Type: "string", Desc: "RFC3339; absent if never run"},
		{Name: "last_run_status", Type: "string", Desc: "status of the most recent run; absent if never run"},
		{Name: "last_success_at", Type: "string", Desc: "RFC3339; absent if never succeeded"},
		{Name: "last_success_age_s", Type: "integer", Desc: "seconds since last success; absent if never succeeded"},
		{Name: "stale", Type: "boolean|null", Desc: "cron-aware: next expected fire after the last success is overdue; null when freshness cannot be computed"},
		{Name: "refreshing", Type: "boolean", Desc: "a non-zombie run is currently active; independent of stale"},
		{Name: "active_run_id", Type: "integer", Desc: "id of the newest active run; absent when none is active, even when a newer terminal overlap row is the latest run"},
		{Name: "active_run_content_digest", Type: "string", Desc: "bundle executed by the active run"},
		{Name: "current_deployment_id", Type: "integer", Nullable: true, Desc: "current successful deployment ID"},
		{Name: "current_app_version", Type: "string", Desc: "current successful deployment version"},
		{Name: "current_content_digest", Type: "string", Desc: "current successful deployment bundle"},
		{Name: "producer_deployment_id", Type: "integer", Nullable: true, Desc: "deployment whose producer most recently published data"},
		{Name: "producer_app_version", Type: "string", Desc: "app version whose producer most recently published data"},
		{Name: "producer_content_digest", Type: "string", Desc: "bundle whose producer most recently published data"},
		{Name: "deploy_trigger_satisfied", Type: "boolean", Desc: "whether the deploy trigger policy is proven for the current bundle"},
		{Name: "producer_repair_required", Type: "boolean", Desc: "true when an uncertain physical write requires successful producer repair, including deploy_trigger=never writers"},
		{Name: "producer_fingerprint", Type: "string", Desc: "fingerprint of the command that last published shared data"},
		{Name: "producer_published_at", Type: "string", Desc: "timestamp when the authoritative producer last published"},
		{Name: "convergence_obligation_id", Type: "integer", Desc: "durable current desired-state obligation"},
		{Name: "convergence_status", Type: "string", Desc: "pending | dispatching | running | satisfied | failed | superseded"},
		{Name: "convergence_run_id", Type: "integer", Desc: "run bound atomically to the current obligation"},
		{Name: "convergence_error", Type: "string", Desc: "last convergence admission or producer error"},
		{Name: "freshness_error", Type: "string", Desc: "present when stale is unknown because the cron or timezone cannot be evaluated"},
	}, EnvelopeFields: []fieldSpec{
		{Name: "items", Type: "array"},
		{Name: "total", Type: "integer"},
		{Name: "limit", Type: "integer"},
	...
	"schedule status": {Mutating: ro, OutputFields: []fieldSpec{
		...
		{Name: "enabled", Type: "boolean"},
		{Name: "last_run_id", Type: "integer", Desc: "id of the most recent run; null if never run; resolved atomically with last_run_status"},
		...
		{Name: "deploy_trigger", Type: "string", Desc: "never | first_deploy | bundle_change"},
		{Name: "deploy_trigger_satisfied", Type: "boolean", Desc: "whether the deploy trigger policy is proven for the current bundle"},
		{Name: "producer_repair_required", Type: "boolean", Desc: "true when an uncertain physical write requires successful producer repair, including disabled writers"},
		{Name: "current_deployment_id", Type: "integer", Nullable: true, Desc: "current successful deployment ID"},
		{Name: "current_app_version", Type: "string", Desc: "current successful deployment version"},
		{Name: "current_content_digest", Type: "string", Desc: "current successful deployment bundle"},
		{Name: "producer_deployment_id", Type: "integer", Nullable: true, Desc: "deployment whose producer most recently published data"},
	...
	"schedule add": {
		...
		OutputFields: []fieldSpec{
			...
			{Name: "name", Type: "string"},
			{Name: "id", Type: "integer"},
			{Name: "convergence_status", Type: "string", Desc: "pending | dispatching | running | satisfied | failed"},
			{Name: "convergence_obligation_id", Type: "integer", Desc: "Durable desired-state obligation"},
			{Name: "convergence_run_id", Type: "integer", Desc: "Run associated with convergence, which may already have completed"},
			{Name: "warning", Type: "string", Desc: "Present when schedule CRUD committed but scheduler/convergence repair remains asynchronous"},
		},
	},
	...
	"schedule retry-convergence": {Mutating: mut, OutputFields: []fieldSpec{
		{Name: "status", Type: "string", Desc: "pending | dispatching | running | satisfied"},
		{Name: "slug", Type: "string"},
		{Name: "name", Type: "string"},
		{Name: "run_id", Type: "integer", Nullable: true, Desc: "run admitted for the retried obligation"},
		{Name: "warning", Type: "string", Nullable: true, Desc: "present when retry was durably accepted but immediate dispatch or readback needs asynchronous repair"},
	}},
	"schedule logs": {
		Mutating:            ro,
		Streaming:           true,
	...
	"fleet apply": {Mutating: mut, Cardinality: "bounded", OutputFields: []fieldSpec{
		...
		{Name: "schedule_verification", Type: "array", Desc: "schedules rejected for enabled cron/convergence policy or unresolved producer-write uncertainty by --verify-schedules"},
		{Name: "schedule_logs", Type: "array", Desc: "relevant failing schedule runs and best-effort log tails"},
		{Name: "warm_restarted", Type: "boolean", Desc: "True when --restart-after-warm cycled this app after bundle data convergence"},
		{Name: "reason", Type: "string", Desc: "human-readable explanation for a skipped or self-healed result"},
