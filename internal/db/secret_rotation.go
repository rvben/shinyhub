...
// has been provisioned, and the usage pseudonym master if initialized.
// reencryptEnv, reencryptCA, and reencryptUsage receive the stored
...
func (s *Store) RotateSecretsTx(reencryptEnv, reencryptCA, reencryptUsage func([]byte) ([]byte, error)) (envRotated int, caRotated, usageRotated bool, err error) {
	...
	if err != nil {
		return 0, false, false, fmt.Errorf("list env vars: %w", err)
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.appID, &r.key, &r.val, &isSecret); err != nil {
			rows.Close()
			return 0, false, false, fmt.Errorf("scan env var: %w", err)
		}
		if isSecret != 0 {
	...
	for _, r := range secretsToRotate {
		...
		if rerr != nil {
			return 0, false, false, fmt.Errorf("re-encrypt env %d/%s: %w", r.appID, r.key, rerr)
		}
		if _, err := tx.Exec(
			`UPDATE app_env_vars SET value = ? WHERE app_id = ? AND key = ?`,
			nv, r.appID, r.key,
		); err != nil {
			return 0, false, false, fmt.Errorf("update env %d/%s: %w", r.appID, r.key, err)
		}
	}
	...
	case caErr != nil:
		return 0, false, false, fmt.Errorf("read worker ca: %w", caErr)
	default:
		nca, rerr := reencryptCA(caEnc)
		if rerr != nil {
			return 0, false, false, fmt.Errorf("re-encrypt worker ca: %w", rerr)
		}
		if _, err := tx.Exec(
			`UPDATE cp_worker_ca SET key_pem_enc = ? WHERE role = ?`,
			nca, workerCARole,
		); err != nil {
			return 0, false, false, fmt.Errorf("update worker ca: %w", err)
		}
		caRotated = true
	}

	// Stable installation key used only to derive app-scoped usage pseudonyms.
	var usageEnc []byte
	usageErr := tx.QueryRow(`SELECT pseudonym_key_enc FROM usage_policy WHERE singleton_id = 1`).Scan(&usageEnc)
	switch {
	case errors.Is(usageErr, sql.ErrNoRows):
		// Usage policy has not been initialized.
	case usageErr != nil:
		return 0, false, false, fmt.Errorf("read usage pseudonym key: %w", usageErr)
	case len(usageEnc) == 0:
		// No pseudonym key is stored yet.
	default:
		nusage, rerr := reencryptUsage(usageEnc)
		if rerr != nil {
			return 0, false, false, fmt.Errorf("re-encrypt usage pseudonym key: %w", rerr)
		}
		if _, err := tx.Exec(`UPDATE usage_policy SET pseudonym_key_enc = ?, updated_at = CURRENT_TIMESTAMP WHERE singleton_id = 1`, nusage); err != nil {
			return 0, false, false, fmt.Errorf("update usage pseudonym key: %w", err)
		}
		usageRotated = true
	}

	if err := tx.Commit(); err != nil {
		return 0, false, false, fmt.Errorf("commit: %w", err)
	}
	return len(secretsToRotate), caRotated, usageRotated, nil
}
