...
PRODUCTS = {"All products": 1.00, "Analytics": 1.18, "Planning": 0.82, "Operations": 1.07}
    ...
    return ui.page_fluid(
        ...
        ui.div(
            ...
            ui.div(
                ui.tags.aside(
                    ui.h2("View filters"),
                    ui.input_select("region", "Region", list(REGIONS), selected="Europe"),
                    ui.input_select("product", "Product", list(PRODUCTS), selected="All products"),
                    ui.input_slider("year", "Reporting year", min=2022, max=2026, value=2026, sep=""),
...
def server(input, output, session):
    register(
        ...
        fields={
            ...
            "forecast": Field("Forecast", formatter=lambda value: "Included" if value else "Actuals only"),
        },
    )
    ...
    def figures():
        ...
        revenue = round(4_250_000 * scale * year_factor * (1.09 if input.forecast() else 1))
    ...
    def dashboard():
        ...
        if not input.forecast():
            growth = growth[:-2]
        bars = [ui.div(class_="trend-bar", style=f"height: {round(value * 100)}%") for value in growth]
        return TagList(
            ui.div(
            ...
            ui.div(
                ui.div(ui.div("Revenue", class_="metric-label"), ui.div(f"€{revenue / 1_000_000:.2f}M", class_="metric-value"), class_="metric"),
                ui.div(ui.div("Gross margin", class_="metric-label"), ui.div(f"{margin:.1%}", class_="metric-value"), class_="metric"),
                ui.div(ui.div("Customers", class_="metric-label"), ui.div(f"{customers:,}", class_="metric-value"), class_="metric"),
                class_="metric-grid",
            ),
            ui.div(
                ui.div(*bars, class_="trend-bars", role="img", aria_label="Synthetic revenue trend"),
                ui.div(ui.span(str(input.year() - len(growth) + 1)), ui.span(str(input.year())), class_="trend-axis"),
                class_="trend",
            ),
        )
