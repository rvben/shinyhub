...
// hosting is enabled.
func startWorkerHosting(ctx context.Context, logger *slog.Logger, cfg *config.Config, store *db.Store) error {
	tokens, err := readJoinTokens(cfg.Worker.JoinTokenFile)
	if err != nil {
		return fmt.Errorf("worker join tokens: %w", err)
	}
	ca, err := worker.OpenCA(cfg.Worker.CADir, tokens)
	if err != nil {
		return fmt.Errorf("worker CA: %w", err)
	}
